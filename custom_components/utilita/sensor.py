...
class UtilitaBalanceSensor(CoordinatorEntity, SensorEntity):
    ...
    def state(self):
        try:
            balance_data = self.coordinator.data.get("balance", {}).get("data", {})
            if "agreements" not in balance_data:
                _LOGGER.warning(f"No 'agreements' key in balance data for {self._supply_type}. Data: {balance_data}")
                return None
            data = balance_data["agreements"]
            for supply in data:
                if supply.get("type") == self._supply_type:
                    balance = supply.get("balance", {})
                    if not isinstance(balance, dict):
                        _LOGGER.error(f"Balance is not a dictionary for {self._supply_type}: {balance}")
                        return None
                    value = Decimal(str(balance.get("money", 0))) / Decimal('100')
                    return float(value.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP))
            _LOGGER.warning(f"No supply found for {self._supply_type} in balance data")
            return None
    ...
    def extra_state_attributes(self):
        try:
            ...
            for supply in data:
                if supply.get("type") == self._supply_type:
                    _LOGGER.debug(f"Supply data for {self._supply_type}: {supply}")
                    balance = supply.get("balance", {})
                    emergency_credit = supply.get("emergency_credit", {})
                    debt = supply.get("debt", {})
                    if not isinstance(balance, dict):
                        _LOGGER.error(f"Balance is not a dictionary for {self._supply_type}: {balance}")
                        return {}
                    if not isinstance(emergency_credit, dict):
                        _LOGGER.error(f"Emergency credit is not a dictionary for {self._supply_type}: {emergency_credit}")
                        return {}
                    if not isinstance(debt, dict):
                        _LOGGER.error(f"Debt is not a dictionary for {self._supply_type}: {debt}")
                        return {}
                    messages = [msg["text"] for msg in balance.get("messages", []) if isinstance(msg, dict)]
                    attrs = {
                        "supply_id": supply.get("supply_id"),
                        "payment_mode": supply.get("payment_mode"),
                        "zero_time": balance.get("zero_time"),
                        "emergency_credit_status": emergency_credit.get("status", "Unknown"),
                        "debt_money": debt.get("money", 0),
                        "debt_recovery_rate": debt.get("debt_recovery_rate", 0),
                        "messages": messages,
                    }
                    if "duration" in balance:
                        attrs["duration_remaining"] = strip_html(balance.get("duration", ""))
                    if "updated" in balance:
                        attrs["updated"] = balance.get("updated")
                    return attrs
            _LOGGER.warning(f"No supply found for {self._supply_type} in balance data")
            return {}
        except (KeyError, TypeError) as err:
...
class UtilitaMessagesSensor(CoordinatorEntity, SensorEntity):
    """Representation of the Utilita unread messages sensor."""
    ...
    @property
    def unique_id(self):
        return f"utilita_{self._entry_id}_messages_unread"

    @property
    def state(self):
        try:
            return self.coordinator.data.get("messages_unread", 0)
        except (KeyError, TypeError) as err:
            _LOGGER.error(f"Error parsing unread messages: {err}")
            return 0
    ...
    def extra_state_attributes(self):
        try:
            ...
            _LOGGER.error(f"Error parsing messages attributes: {err}")
            return {}
...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    sensors.extend([
        ...
        UtilitaMessagesSensor(coordinator, entry_id),
    ])
